...
class MDLP_Discretizer(object):
    ...
    def apply_cutpoints(self, X, out_data_path=None, out_bins_path=None):
        '''
        ...
        for attr in self._features:
            if len(self._cuts[attr]) == 0:
                ...
                X[attr] = pd.cut(x=X[attr].values, bins=cuts, right=False, labels=bin_labels,
                                          precision=6, include_lowest=True)

        #reconstitute full data, now discretized
        # if self._ignored_features:
        #     to_return = None #pd.concat([X, self._data_raw[list(self._ignored_features)]], axis=1)
